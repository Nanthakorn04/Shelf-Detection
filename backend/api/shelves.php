    ...
    if ($method === 'POST') {
        ...
        if ($action === 'update_inventory') {
            ...
            if (array_key_exists('quantity', $in)) {
                $oldStmt = $db->prepare("SELECT quantity FROM shelf_inventory WHERE id = ?");
                $oldStmt->execute([$invId]);
                $oldRow = $oldStmt->fetch();
                $oldQty = $oldRow ? (int) $oldRow['quantity'] : null;
            }
            ...
            $stmt->execute($params);
            if ($oldQty !== null) {
                notifyInventoryChange($db, $invId, $oldQty);
            }
